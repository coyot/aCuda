...
Na początek, skan bazy danych $DB$ tworzy \emph{listę elementów częstych}: $\lbrace (f, 4), (c, 4), (a, 3),$ $ (b, 3), (m, 3), (p, 3) \rbrace$ (pary reprezentują odpowiednio - element oraz liczbę jego wystąpień), w której elementy posortowane są zgodnie z malejącą liczbą wystąpień. Sposób sortowania jest istotny, ponieważ proces budowy drzewa będzie właśnie oparty na tym porządku. Dla wygody analizy oraz łatwiejszego zrozumiania, pierwszą kolumnę z prawej w tablicy~\ref{example:fp_data} tworzą elementy częste z danej transakcji, posortowane właśnie w ten sposób.
...
\begin{enumerate}
	\item Skan pierwszej transakcji ($TID = 100$) prowadzi do wprowadzenia w drzewie pierwszej gałęzi: $\lbrace (f, 1), (c, 1), (a, 1), (m, 1), (p, 1) \rbrace$. Warto zauważyć, że elementy częste z danej transakcji są wylistowane zgodnie z porządkiem w \emph{liście elementów częstych}.
	...
	\item Skan czwartej transakcji ($TID = 400$) prowadzi do utworzenia nowej gałęzi drzewa $\lbrace (c, 1),$ $(b, 1), (p, 1) \rbrace$, ponieważ transakcja ta nie współdzieli żadnego fragmentu z istniejącą już ścieżką w FP-drzewie.
		...
		\li \If $\id{Tree}$ zawiera jedną ścieżkę prefiksową
			...
			\li \Else 
				...
				\li \For \textbf{each} element $a_i \in \id{Q}$
					...
					\li \textbf{stwórz} warunkową bazę prefiksów z $\beta$ oraz FP-drzewo ($Tree_{\beta}$) dla $\beta$
						\li	\If $Tree_{\beta} \neq \emptyset$
							\li \Then
...
Przez \emph{warunkowea baza prefiksów} (\english{conditional pattern base}) rozumie się podprefiks pod warunkiem istnienia danego prefiksu, pod warunkiem występowania elementu podstawowego.

\subsubsection{Przykład eksploracji FP-drzewa}
W rozdziale~\ref{sec:example_fptree} zaprezentowano
